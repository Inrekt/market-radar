 ...
 * она честно живёт на новом уровне. За кадром оказывается только короткий
 * участок — двадцатая часть окна, три минуты часа. Всё, что длиннее, — уже не
 * выброс, а поведение рынка, и прятать его нельзя ни ради какого масштаба:
 * у ушедшей за край минуты форма не читается совсем, остаётся одно число.
 */
const CVD_TRIM_MAX_FRAC = 0.05
/**
 * Порог, за которым обрезка окупается. На целой шкале основная часть линии
 * занимает ту долю высоты панели, которую ей оставил вынос; обрезка отдаёт ей
 * всю панель, то есть увеличивает форму в 1/долю раз. Половина выбрана так,
 * чтобы обрезка как минимум УДВАИВАЛА место для остального часа: ради 1.5× не
 * стоит уводить за край даже минуту, а нетронутая шкала всегда честнее любой
 * пометки. Прогон по архиву (1422 часовых окна, 17 монет): порог срабатывает на
 * 2% карточек, и на них форма вырастает в 2.0–7.7 раза.
 */
const CVD_BULK_MIN_FRAC = 0.5
/**
 * Запас при выборе, СКОЛЬКО точек увести за край. Размах падает с каждой
 * отброшенной точкой всё медленнее, и последние из них уже почти ничего не
 * дают. Берётся самая ранняя обрезка, которая укладывается в этот запас от
 * лучшей достижимой: 10% размаха — плата за то, чтобы за кадром осталось на
 * треть меньше времени.
 */
const CVD_TRIM_SLACK = 1.1
/**
 * Отступ подписи «ушло за край» от самого участка. Подпись стоит СБОКУ от него,
 * а не над ним: над ним по краю панели идёт прижатая линия, и текст оказывался
 * перечёркнут ею насквозь.
 */
const CVD_LABEL_GAP_PX = 6
 ...
 * пятидесяти пяти минут сплющивается в нитку. Замер SOL 22:16: вынос длиной в
 * полминуты увёл CVD на $14.4M и занял почти всю панель, а остальные 57 минут
 * ужались в её нижнюю четверть.
 *
 * Шкала — самый узкий интервал, вмещающий почти все точки. Взят он не
 * фиксированным перцентилем: перцентиль режет симметрично, а вынос почти всегда
 * односторонний (за час CVD уходит либо вниз, либо вверх), и половина обрезки
 * тратилась бы на ту сторону, где ничего нет. Скользящее окно по отсортированным
 * значениям само сдвигается туда, где точки стоят плотно.
 *
 * Решение принимается в два шага, и оба нужны.
 * 1. Стоит ли вообще трогать шкалу: если даже предельная обрезка не отдаёт
 *    остальной линии вдвое больше высоты, шкала остаётся целой. Гладкий тренд
 *    сюда и попадает — сколько у него ни отрезай, размах падает пропорционально,
 *    и на ровных данных карточка ведёт себя ровно как раньше.
 * 2. Сколько именно точек увести за край: самая ранняя обрезка, которая почти
 *    догоняет предельную (CVD_TRIM_SLACK). Отбрасывать до упора незачем —
 *    последние точки размах почти не сужают, а времени за кадром прибавляют.
 *
 * Обрезанный участок НЕ пропадает: линия прижимается к краю, а cvdOffScale
 * ставит под ней полосу и подписывает достигнутое значение. Сами данные при
 * этом не трогаются — меняется только шкала.
 */
function cvdDomain(values: readonly number[]): CvdDomain | null {
  ...
  // Самое узкое окно из keep подряд идущих значений — по одному на каждое число
  // отброшенных точек. Где бы ни сидел вынос, окно уедет от него само.
  const options: CvdDomain[] = [{ lo, hi, clipped: false }]
  ...
  for (let drop = 1; drop <= maxDrop; drop++) {
    ...
    for (let i = 1; i + keep <= count; i++) {
      ...
      if (to - from < bestHi - bestLo) {
        bestLo = from
        bestHi = to
      }
    }
    options.push({ lo: bestLo, hi: bestHi, clipped: true })
  }

  const limit = options[options.length - 1]!
  // Шаг 1: предельная обрезка не окупается — шкала остаётся целой.
  if ((limit.hi - limit.lo) / full >= CVD_BULK_MIN_FRAC) return { lo, hi, clipped: false }
  // Шаг 2: самая ранняя обрезка, почти догнавшая предельную.
  const enough = (limit.hi - limit.lo) * CVD_TRIM_SLACK
  return options.find((option) => option.clipped && option.hi - option.lo <= enough) ?? limit
}
...
function deltaBars(
  ...
  const sizes = buckets
    .map((bucket) => Math.abs(bucket.buyUsd - bucket.sellUsd))
    .filter((value) => value > 0)
    .sort((a, b) => a - b)
  if (sizes.length === 0) return []

  // Шкала по перцентилю, а не по максимуму. Один вынос за десять секунд бывает
  // в разы больше всего остального часа: при масштабе по нему все прочие
  // столбики ложатся в нитку у нуля, и панель перестаёт что-либо показывать.
  // Ровно та же болезнь, что лечили у линии накопленной дельты.
  const peak = sizes[Math.min(sizes.length - 1, Math.floor(sizes.length * BAR_SCALE_PERCENTILE))] ?? 0
  if (peak === 0) return []
  ...
  for (const bucket of buckets) {
    ...
    if (delta === 0) continue
    const raw = (Math.abs(delta) / peak) * half
    const over = raw > half
    const h = Math.max(MIN_BAR_PX, Math.min(raw, half))
    const up = delta > 0
    bars.push({
      x: timeX(bucket.t),
      y: up ? zeroY - h : zeroY,
      w: barW,
      h,
      c: up ? theme.bull : theme.bear,
    })
    // Столбик, не поместившийся в шкалу, получает засечку у самого края.
    // Молча обрезать нельзя: обрезанный выглядит как обычный крупный, и вынос,
    // ради которого на карточку и смотрят, теряется среди соседей.
    if (over) {
      bars.push({
        x: timeX(bucket.t) - BAR_OVER_SPREAD_PX,
        y: up ? zeroY - h - BAR_OVER_MARK_PX : zeroY + h,
        w: barW + 2 * BAR_OVER_SPREAD_PX,
        h: BAR_OVER_MARK_PX,
        c: up ? theme.bull : theme.bear,
      })
    }
...
/**
 * Перцентиль, по которому масштабируются столбики дельты. 0.97 оставляет за
 * бортом примерно один столбик из тридцати трёх — этого хватает, чтобы редкий
 * вынос не решал за всю панель, и мало, чтобы обрезанных было заметно много.
 */
const BAR_SCALE_PERCENTILE = 0.97
/** Толщина засечки над не поместившимся столбиком. */
const BAR_OVER_MARK_PX = 2
/** На сколько засечка шире самого столбика — чтобы читалась как пометка. */
const BAR_OVER_SPREAD_PX = 1
...
function cvdOffScale(
  ...
  const labels: Label[] = []
  /** Подпись сбоку от того участка, на котором линия ушла дальше всего. */
  const mark = (runs: readonly OffRun[], x: number, text: string, y: number): void => {
    const run = runs.find((candidate) => x >= candidate.x0 && x <= candidate.x1)
    // Ширина по оценке CHAR_W_PX: настоящую знает только браузер, а решение
    // «справа или слева» нужно уже здесь.
    const width = text.length * CHAR_W_PX
    const right = (run?.x1 ?? x) + CVD_LABEL_GAP_PX
    if (right + width <= frame.plotX1) {
      labels.push({ x: right, y, s: text, a: 'left' })
      return
    }
    // Справа не поместилось — подпись уходит влево от участка и упирается в левый
    // край окна, а не вылезает за него.
    const left = Math.max(frame.plotX0 + width, (run?.x0 ?? x) - CVD_LABEL_GAP_PX)
    labels.push({ x: left, y, s: text, a: 'right' })
  }
  ...
  if (below.length > 0) {
    mark(below, worstLowX, `CVD ниже кадра ${signedMoney(worstLow)}`, frame.lowY1 - CVD_INSET_PX - LABEL_FONT_PX)
  }
  if (above.length > 0) {
    mark(above, worstHighX, `CVD выше кадра ${signedMoney(worstHigh)}`, frame.lowY0 + CVD_INSET_PX + LABEL_FONT_PX)
  }
  return { rects, labels }
}
